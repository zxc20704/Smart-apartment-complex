...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "fix_edit")) {
  $updateSQL = sprintf("UPDATE fix SET Name=%s, Location=%s, `Date`=%s, Detail=%s, Member=%s WHERE Id=%s",
...
$colname_cn_fix = "-1";
if (isset($_GET['Id'])) {
  $colname_cn_fix = $_GET['Id'];
}
mysql_select_db($database_cn_tanzi, $cn_tanzi);
$query_cn_fix = sprintf("SELECT * FROM fix WHERE Id = %s", GetSQLValueString($colname_cn_fix, "int"));
$cn_fix = mysql_query($query_cn_fix, $cn_tanzi) or die(mysql_error());
$row_cn_fix = mysql_fetch_assoc($cn_fix);
$totalRows_cn_fix = mysql_num_rows($cn_fix);
?>
<!doctype html>
<html>
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/less.js/2.7.1/less.js"></script>
        ...
        <div class="box">
            ...
            <div id="collapse4" class="body">
              <form action="<?php echo $editFormAction; ?>" id="fix_edit" name="fix_edit" method="POST">
                <table width="70%" border="0" cellspacing="0" cellpadding="0">
                  <tbody>
                    <tr>
                      <td><label for="Name">需維修物品:</label>
                      <input name="Name" type="text" id="Name" value="<?php echo $row_cn_fix['Name']; ?>"></td>
                    </tr>
                    <tr>
                      <td><label for="Location">需維修物品位置:</label>
                      <input name="Location" type="text" id="Location" value="<?php echo $row_cn_fix['Location']; ?>"></td>
                    </tr>
                    <tr>
                      <td><label for="Date">通報時間:</label>
                      <input name="Date" type="text" id="Date" value="<?php echo $row_cn_fix['Date']; ?>"></td>
                    </tr>
                    <tr>
                      <td><label for="Member">通報帳號:</label>
                      <input name="Member" type="text" id="Member" value="<?php echo $row_cn_fix['Member']; ?>"></td>
                    </tr>
                     <tr>
                      <td><label for="Detail">損壞情況:</label>
                      <textarea id="Detail" class="ckeditor" name="Detail" value="<?php echo $row_cn_fix['Detail']; ?>"><?php echo $row_cn_fix['Detail']; ?></textarea>
                ...
                <input name="Id" type="hidden" id="Id" form="fix_edit" value="<?php echo $row_cn_fix['Id']; ?>">
                <input type="hidden" name="MM_update" value="fix_edit">
              </form>
              <p>&nbsp;</p>
