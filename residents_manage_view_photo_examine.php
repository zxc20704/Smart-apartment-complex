...
if (isset($_GET['Login_id'])) {
  $colname_cn_member_photo = $_GET['Login_id'];
}
mysql_select_db($database_cn_tanzi, $cn_tanzi);
...
/*mysql_select_db($database_cn_tanzi, $cn_tanzi);
$query_cn_member_photo = sprintf("SELECT * FROM `member_photo` WHERE Login_id = %s", GetSQLValueString($colname_cn_member_photo, "text"));
$cn_member_photo = mysql_query($query_cn_member_photo, $cn_tanzi) or die(mysql_error());
$row_cn_member_photo = mysql_fetch_assoc($cn_member_photo);
$totalRows_cn_member_photo = mysql_num_rows($cn_member_photo);*/
?>
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="assets/lib/123/123.css">
                                           ...
                                           <?php
                                                ...
                                                if ($dh = opendir($dir)){
                                                    while (($file = readdir($dh))!= false){
                                                        //文件名的全路径 包含文件名
                                                        $filePath = $dir.$file;
                                                        echo "<img src='".$filePath."' width='240' height='240'  alt=''/>".$file;
                                                        //echo "<img src='".$filePath."'/>".$file;      放大解析度降低
