...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE votemain SET titleNo=%s WHERE itemNum=%s",
...
if (isset($_GET['titleNo'])) {
  $colname_govoteRec = $_GET['titleNo'];
}
mysql_select_db($database_cn_tanzi, $cn_tanzi);
$query_govoteRec = sprintf("SELECT * FROM votemain WHERE titleNo = %s", GetSQLValueString($colname_govoteRec, "int"));
$govoteRec = mysql_query($query_govoteRec, $cn_tanzi) or die(mysql_error());
$row_govoteRec = mysql_fetch_assoc($govoteRec);
$totalRows_govoteRec = mysql_num_rows($govoteRec);

$colname_voteTitle = "-1";
if (isset($_GET['titleNo'])) {
  $colname_voteTitle = $_GET['titleNo'];
}
mysql_select_db($database_cn_tanzi, $cn_tanzi);
$query_voteTitle = sprintf("SELECT * FROM votetitle WHERE titleNo = %s", GetSQLValueString($colname_voteTitle, "int"));
$voteTitle = mysql_query($query_voteTitle, $cn_tanzi) or die(mysql_error());
$row_voteTitle = mysql_fetch_assoc($voteTitle);
$totalRows_voteTitle = mysql_num_rows($voteTitle);

if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")){
	$updateSQL="UPDATE votemain SET itemNum=itemNum+1 WHERE itemNo=".$_POST['radio'];
	
	MySQL_select_db($database_cn_tanzi, $cn_tanzi);
	$Result1=MySQL_query($updateSQL, $cn_tanzi) or die(MySQL_error());
	
	$updateGoTo="container8_3.php";
	if(isset($_SERVER['QUERY_STRING'])){
...
<html>
    <head>
        ...
        <title>社區管理站</title>
         <style type="text/css">
          ...
          .style9 {font-size: 10pt; color: #FFFFFF; }
          .style27 {
	      font-size: 10pt;
	      color: #666666;
          }
  ...
  <tr>
    ...
    <td>票選主題:<img src="webVote/img/fast-forward.gif" width="16" height="16" align="absmiddle" /> <?php echo $row_voteTitle['voteTitle']; ?>
<form action="<?php echo $editFormAction; ?>" id="form1" name="form1" method="POST">
        <table width="650" border="0" align="center" cellpadding="2" cellspacing="2">
          <tr>
            <?php do { ?>
              <td width="81" align="right"><label>
                
                <input name="radio" type="radio" id="voteItem" value="<?php echo $row_govoteRec['itemNo']; ?>" checked="checked" />
                
              </label></td>
              <td width="555"><?php echo $row_govoteRec['itemName']; ?></td>
              <?php } while ($row_govoteRec = mysql_fetch_assoc($govoteRec)); ?>
          </tr>
          <tr>
            <td >
 			 <br>			
             <input type="image" name="submit" id="submit" src="webVote/img/dovote.gif" />
             <br>
             <a href="container8_1.php" ><br>
              <img src="webVote/img/votemain.gif" alt="" width="80" height="24" border="0" /></a>
              </td>
          </tr>
        </table>
  ...
  <tr>
    <td><img src="webVote/img/atable22-6.gif" width="29" height="11" /></td>
   <td background="webVote/img/atable22-7.gif"><input name="MM_update" type="hidden" id="MM_update" value="forml" />
